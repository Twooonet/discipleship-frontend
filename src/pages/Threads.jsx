...
export default function Threads() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      {/* New thread form */}
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-4 space-y-3">
          <h2 className="font-semibold text-gray-700">Share your thoughts</h2>
          <div>
        ...
        <div className="space-y-3">
          {threads.map(thread => (
            <Link
              ...
              key={thread.id}
              className="block bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-md hover:border-primary-100 transition-all"
            >
              <div className="flex items-start justify-between gap-2">
                <p className="font-semibold text-gray-800 line-clamp-1 flex-1">{thread.title}</p>
                {isAdmin && (
                  <button
